...
def gangChat(client, username, groupname):
    while True:
...
def handshake(client):
    ...
    if groupname in groups:
        if username in groups[groupname].allMembers:
            ...
            groups[groupname].sendMessage(username + " has requested to join the group.", "gangChat")
            client.send(b"/wait")
            print("Join Request:", username, "| Group:", groupname)
        threading.Thread(target=gangChat, args=(client, username, groupname,)).start()
    else:
...
def main():
    if len(sys.argv) < 3:
        ...
        print("EXAMPLE: server.py localhost 6969")
        return
    listenSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    listenSocket.bind((sys.argv[1], int(sys.argv[2])))
    listenSocket.listen(10)
    print("gangChat Server running")
    while True:
